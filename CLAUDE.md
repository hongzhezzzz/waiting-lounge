...
12. **Customer-facing test principle.** When asking the user to test something Claude built, the test should feel like a customer using a polished product — no "command not found", no missing-setup dead-ends, no unrun `npm link`, no env vars Claude assumed. Pre-walk the path Claude is about to ask the user to walk; if any step would fail in their actual environment, fix it FIRST. Reserve user tests for things genuinely needing their input (visual UX feel, real claude process, real human interaction). Even those tests should arrive at the user's hands with polished setup + clean instructions. A user test that fails due to a setup issue Claude could have caught is a Claude failure, not a product issue. The user's framing (locked 2026-05-08): "Clean the friction obstacle, and I will test it afterwards. You should prioritize each test with me as a 'customer facing' test, unless you are stuck and needs input."
